...
namespace GoogleCloudSamples
{
    public partial class Dlp
    {
        ...
        class NumericalStatsOptions
        {
            ...
            [Value(4, HelpText = "The name of the column to compute risk metrics for. (e.g. 'age')", Default = 0)]
            public string ColumnName { get; set; }

            [Value(5, HelpText = "The name of the Pub/Sub topic to notify once the job completes.", Default = 0)]
            public string TopicId { get; set; }

            [Value(6, HelpText = "The name of the Pub/Sub subscription to use when listening for job completion notifications.", Default = 0)]
            public string SubscriptionId { get; set; }
        }

        [Verb("categoricalStats", HelpText = "Computes risk metrics of a column of data in a Google BigQuery table.")]
        class CategoricalStatsOptions
        {
            [Value(0, HelpText = "The project ID to run the API call under.", Required = true)]
            public string CallingProjectId { get; set; }
            ...
            [Value(6, HelpText = "The name of the Pub/Sub subscription to use when listening for job completion notifications.", Required = true)]
            public string SubscriptionId { get; set; }
        }

        [Verb("kAnonymity", HelpText = "Computes the k-anonymity of a column set in a Google BigQuery table.")]
        class KAnonymityOptions
        {
            [Value(0, HelpText = "The project ID to run the API call under.", Required = true)]
            public string CallingProjectId { get; set; }
        ...
        class LDiversityOptions
        {
            ...
            [Value(4, HelpText = "The column to measure l-diversity relative to. (e.g. 'age')", Required = true)]
            public string SensitiveAttribute { get; set; }

            [Value(5, HelpText = "A set of columns that form a composite key, delimited by commas. (e.g. 'name,city')", Required = true)]
            public string QuasiIdColumns { get; set; }

            [Value(6, HelpText = "The name of the Pub/Sub topic to notify once the job completes.", Required = true)]
            public string TopicId { get; set; }

            [Value(7, HelpText = "The name of the Pub/Sub subscription to use when listening for job completion notifications.", Required = true)]
            public string SubscriptionId { get; set; }
        }
        ...
        class KMapOptions
        {
            ...
            [Value(6, HelpText = "A list of the infoTypes for each quasi-id, delimited by commas.", Required = true)]
            public string InfoTypes { get; set; }

            [Value(7, HelpText = "The name of the Pub/Sub topic to notify once the job completes.", Required = true)]
            public string TopicId { get; set; }

            [Value(8, HelpText = "The name of the Pub/Sub subscription to use when listening for job completion notifications.", Required = true)]
            public string SubscriptionId { get; set; }

            [Value(9, HelpText = "The ISO 3166-1 region code that the data is representative of.", Default = "")]
    ...
    opts.SubscriptionId);
            ...
            for (int bucketIdx = 0; bucketIdx < result.KMapEstimationHistogram.Count; bucketIdx++)
            {
                var bucket = result.KMapEstimationHistogram[bucketIdx];
                Console.WriteLine($"Bucket {bucketIdx}");
                Console.WriteLine($"  Anonymity range: [{bucket.MinAnonynity}, {bucket.EquivalenceClassSizeUpperBound}].");
                Console.WriteLine($"  {bucket.BucketSize} unique values total.");
